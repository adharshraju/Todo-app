...
export class MockTodoService implements TodoService
{
    ...
    public constructor()
    {
        ...
        for (let i = 0; i < count; i++)
            todos.push(new MockTodoProxy("id" + i, "Title " + i, "Description " + i, "2023-02-2" + i));
    ...
    public createTodo(title: string, description: string, date: string): Promise<Todo>
    {
        given(title, "title").ensureHasValue().ensureIsString();
        given(description, "description").ensureIsString();
        given(date, "date").ensureIsString();

        const todo = new MockTodoProxy("id" + this._counter++, title.trim(), description, date);
